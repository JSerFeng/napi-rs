...
const platformArchTriples = mapValues(groupBy(Object.values(tripleLists).filter((k) => SUPPORTED_PLATFORM.has(k.platform)), 'platform'), (v) => groupBy(v, 'arch'));
const fileContent = `
module.exports.platformArchTriples = ${JSON.stringify(platformArchTriples)}
`;
writeFileSync(join(__dirname, 'triples', 'index.js'), esbuild.transformSync(fileContent, {
    minify: true,
}).code);
//# sourceMappingURL=generate-triple-list.js.map
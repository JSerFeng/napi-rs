...
export const createGithubActionsCIYml = (binaryName, targets) => {
    ...
    fullTemplate.jobs.build.strategy.matrix.settings =
        fullTemplate.jobs.build.strategy.matrix.settings.filter(({ target }) => targets.includes(target));
    ...
    if (!enableWindowsX86 && !enableMacOSX86) {
        ...
        fullTemplate.jobs[TEST_MACOS_WINDOWS].strategy.matrix.settings =
            fullTemplate.jobs[TEST_MACOS_WINDOWS].strategy.matrix.settings.filter(({ host }) => host !== filterTarget);

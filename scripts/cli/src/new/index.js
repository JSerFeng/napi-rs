...
export class NewProjectCommand extends Command {
    constructor() {
        ...
        this.dirname = Option.String({
            name: '-d,--dirname',
            required: false,
        });
        this.targets = Option.Array('--targets,-t');
        this.dryRun = Option.Boolean(`--dry-run`, false);
        this.enableGithubActions = Option.Boolean(`--enable-github-actions`);
    }
    execute() {
        var _a, _b;
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!this.dirname) {
                ...
                const dirAnswer = yield inquirer.prompt({
                    type: 'input',
            ...
            if (!this.targets) {
                const { targets } = yield inquirer.prompt([
                    {
                        type: 'checkbox',
                        name: 'targets',
            ...
            this.writeFile('__test__/index.spec.mjs', `import test from 'ava'
...
`);
            if (this.enableGithubActions) {
                ...
                this.writeFile(join('.github', 'workflows', 'CI.yml'), createGithubActionsCIYml(binaryName, this.targets));
            }
            yield CreateNpmDirCommand.create('package.json', join(process.cwd(), this.dirname), join(process.cwd(), this.dirname));
...
`);
    ...
    getName() {
        return __awaiter(this, void 0, void 0, function* () {
            if (!this.name) {
                const nameAnswer = yield inquirer.prompt({
                    type: 'input',
...
NewProjectCommand.usage = Command.Usage({
